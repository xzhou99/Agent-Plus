...
float findMinCost(int max_number_of_nodes)
{
	...
	root->CalculateCost();
	BBDebugfile << "Find an active node " << root->node_id << " with a  UB cost = " << root->l_vrp_data.UBCost << " LB cost = " << root->l_vrp_data.LBCost << endl;
	...
	while (!pq.empty())
	{
		...
		// live nodes
		if (min->level == cur_level)
		{
			if (level_counter > N_BEAM_RANGE)
			{
				pq.pop();
				continue;
			}
		...
		else if (min->level < cur_level)
		{
			pq.pop();
			continue;
		}
		else //min->level > cur_level
		{
			continue;
		}

		pq.pop();
		cout << "branch node " << level_counter << "at layer " << cur_level << endl;
			...
			if (competing_vehicle_size >= 2) // if there are more than 2 competing vehicles
			{
				...
				for (int vi = 0; vi < competing_vehicle_size; vi++)
				{
					...
					for (int vj = 0; vj < competing_vehicle_size; vj++)
					{
					  ...
					  { 
						  int new_vehicle_id = min->l_vrp_data.V2PAssignmentVector[p].output_competting_vehicle_id_vector[vj];
			//			  child->VehicleID = veh_id;
						  child->l_vrp_data.AddProhibitedAssignment(p,
							  min->l_vrp_data.V2PAssignmentVector[p].input_prohibited_vehicle_id_vector,
				...
				{ 
					int new_vehicle_id = min->l_vrp_data.V2PAssignmentVector[p].output_competting_vehicle_id_vector[vj];
					child->l_vrp_data.AddProhibitedAssignment(p, min->l_vrp_data.V2PAssignmentVector[p].input_prohibited_vehicle_id_vector, new_vehicle_id);
				}
